...
export function getCurrentUser() {
  ...
  // 로컬 Fallback 유저 (Firebase 미구동 시)
  if (typeof window !== 'undefined') {
    const localUser = localStorage.getItem('classboard_local_user');
    if (localUser) {
...
export function initSampleData() {
  ...
  setLocal(KEYS.AN, [
    { id: genId(), title: '📌 중간고사 일정 안내', content: '이번 학기 중간고사는 5월 25일(월)~29일(금)입니다.\n시험 범위는 각 과목 선생님께 확인해 주세요.\n모두 화이팅! 💪', authorId: 'user_01', authorName: '로컬 학습자', createdAt: new Date(Date.now() - 86400000).toISOString() },
